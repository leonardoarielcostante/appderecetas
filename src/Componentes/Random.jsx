...
export default function Random(){
  ...
  const ingredients = Object.entries(randomMeal).filter(([key, data]) => key.includes("strIngredient") && data !== "").map(([key, data]) => data);
  const amount = Object.entries(randomMeal).filter(([key, data]) => key.includes("strMeasure") && data !== "").map(([key, data]) => data);
  ...
  return(
    <section className="flex flex-col items-center justify-center">
      <div className="relative w-full h-72 lg:h-96 max-w-xl sm:mt-6">
        <h2 className="absolute bottom-0 z-10 w-full text-center bg-[#45D1F4] bg-opacity-80 text-2xl p-1 lg:text-3xl">{randomMeal.strMeal}</h2>
        <img className="h-72 lg:h-96 w-full object-cover relative z-0" alt={'imagen de' + randomMeal.strMeal} src={randomMeal.strMealThumb} />
      </div>
      <div className="pt-2">
        <p className="sm: text-2xl lg:text-3xl">{randomMeal.strArea}, {randomMeal.strCategory}</p>
      </div>
        ...
        <div className="flex flex-col w-72 items-center justify-evenly bg-white shadow-[#091A1E] shadow border border-[#45D1F4] rounded mb-4 md:mr-4">
          ...
          <div className="flex flex-col mb-2 w-72 items-start h-full p-1 border border-[#45D1F4] rounded bg-white text-2xl shadow-[#091A1E] shadow sm:w-auto ">
            <h3 className=" border-b">Instructions</h3>
              <p className="text-sm mt-2 lg:text-base">{randomMeal.strInstructions}
              </p>
          </div>
      </div>
    </section>
